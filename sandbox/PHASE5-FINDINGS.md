...
DevLoop was built against **selran-mail-v4 (Rust backend)**. Phase 5 ran it on
**Cortex (Python backend + Vite/Playwright frontend)** — a different stack — by
adding **one file**: `Cortex/.devloop/autoloop.toml`. Zero changes to `skills/`,
`sandbox/`, `driver/`, or `cli/`.

```
$ cli/autoloop run /Users/aidin/NeutronDev/Cortex --dry-run
[autoloop] repo=…/Cortex base=main-v2 branch=claude/autoloop-20260520-163738 dry_run=1
...
Verified afterwards: Cortex still on `main-v2` at the same commit, the autoloop
branch deleted, and the only change is the untracked `.devloop/autoloop.toml`. The
pre-existing Cortex Cowork worktree was untouched.
...
- Cortex's `.devloop/autoloop.toml` was left **untracked** in the Cortex repo (a
  reversible, config-only change) — commit it there when you want Cortex permanently
  onboarded. The launch/health/db fields carry `CUSTOMIZE` markers; fill them from
  Cortex's actual server before a live run.
- Cortex already ships frontend e2e (`frontend/playwright.config.ts`, `tests-e2e/`),
