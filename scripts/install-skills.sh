#!/usr/bin/env bash
# Install the four DevLoop skills into Claude Code's skill directory so the loop
...
for skill in cartographer app-audit audit-fix pre-commit-verification; do
  if [[ ! -d "$SRC/$skill" ]]; then
