...
## Impact analysis queries ("what's impacted if I change X?")

The codemap answers change-impact questions directly — no new build, just a
graph walk over data already on disk. Trigger phrases: "what's impacted if I
change X", "what depends on X", "what breaks if I remove X", "impact analysis
on X".

- **Forward impact (who depends on X):** BFS from X over
  ...
  in the answer and offer to go deeper.
- **Backward impact (what X relies on):** BFS over `calls` / `imports` — what
  must hold for X to be correct. Useful before trusting X as a fix point.
- **Join spec_refs** onto every impacted function so the answer reads as
  intent, not just file count: "changing `event_bus.emit` reaches 14 files
  across 2 levels, implementing §3.1 (auth events) and §6.2 (sync ordering)."
- **Format:** one level per block, each entry as `file:function — via <edge>`,
...
Honesty (same as the limits below): the static graph misses dynamic dispatch,
